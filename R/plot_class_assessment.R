#' Plot discrete class assessment results for selected lakes
#'
#' Draws a class-by-year heatmap for selected lakes from a
#' [discreteClassAssessment()] result.
#'
#' @param class_assessment A list created by [discreteClassAssessment()].
#' @param lakeNames Optional character vector of lake names to plot.
#' @param lakeIDs Optional character vector of lake IDs to plot.
#' @param rowNumbers Optional numeric vector of row numbers in the assessment
#'   table to plot.
#'
#' @return No explicit return value. The function draws a base R plot.
#'
#' @details At least one of `lakeNames`, `lakeIDs`, or `rowNumbers` must identify
#'   rows. Colors are generated by [rescale_classColors()].
#'
#' @importFrom grDevices rgb col2rgb
#' @importFrom graphics axis rect text par plot
#' @export
#' 
plot_class_assessment <- function(
  ...
  for(i in 1:n_lakes){
    for(j in class_cols){
      waterClass <- as.character(df_reverse[i,j])
...
#' Rescale the default class color table
#'
#' Adapts `lakeRS::tenClassColors` to an arbitrary number of classes for class
#' assessment plots.
#'
#' @param nClass Integer. Number of classes required.
#'
#' @return A data frame with columns `class` and `color`, containing exactly
#'   `nClass` rows.
#'
#' @details If fewer than ten classes are requested, colors are sampled from the
#'   default ten-class palette. If more than ten classes are requested,
#'   additional colors are interpolated between neighboring palette entries.
#'
#' @importFrom grDevices rgb col2rgb
#' @keywords internal
#' 
